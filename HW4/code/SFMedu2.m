...
% addpath(genpath('../xinshuo_toolbox'));

visualize = false;
adjust_type = 'all';        % structure, motion, s+m, focal, all
smart_mering = true;
%% data


% frames.images{1}='data/roman/images/B21.jpg';
% frames.images{2}='data/roman/images/B22.jpg';
% frames.images{3}='data/roman/images/B23.jpg';
% frames.images{4}='data/roman/images/B24.jpg';
% frames.images{5}='data/roman/images/B25.jpg';

% frames.images{1}='data/elephant/images/1.JPG';
% frames.images{2}='data/elephant/images/2.JPG';
% frames.images{3}='data/elephant/images/3.JPG';
% frames.images{4}='data/elephant/images/4.JPG';
% frames.images{5}='data/elephant/images/5.JPG';


frames.images{1}='data/books/images/1.JPG';
frames.images{2}='data/books/images/2.JPG';
frames.images{3}='data/books/images/3.JPG';
frames.images{4}='data/books/images/4.JPG';
frames.images{5}='data/books/images/5.JPG';
...
try
    ...
    fprintf('get focal length from list\n');
catch
end
if ~isfield(frames,'focal_length') || isempty(frames.focal_length)
    ...
    fprintf('get focal length by default\n');           % run here
end
...
for frame=1:frames.length-1    
    ...
    pair = match2viewSIFT(frames, frame, frame+1);                                                              % match point correspondences using SIFT
    ...
    if true % choose between different ways of getting E
        ...
        pair = estimateF(pair);                                                                                 % estimate fundamental matrix using point correspondences
        % Convert Fundamental Matrix to Essential Matrix
        pair.E = frames.K' * pair.F * frames.K; % MVG Page 257 Equation 9.12                                    % obtain essential matrix
    else
    ...
    pair.Rt = RtFromE(pair,frames);                                                                             % decompose essential matrix to R and t
    
    % Convert the pair into the BA format
    Graph{frame} = pair2graph(pair,frames);
    
    % re-triangulation
    Graph{frame} = triangulate(Graph{frame},frames);                                                            % triangulation
    ...
    % bundle adjustment
    Graph{frame} = bundleAdjustment(Graph{frame}, adjust_type);                                                              % bungle adjustment
...
if smart_mering
    ...
    % select the root graph with most number of matches
    num_match_most = 0;
    start_frame = 1;
    graph_queue = 1:frames.length-1;
    delete_index = 0;
    for frame_index = 2:length(graph_queue)-1
        graph_tmp = graph_queue(frame_index);

        match_tmp = Graph{graph_tmp}.num_match;
        if match_tmp > num_match_most
            num_match_most = match_tmp;
            start_frame = graph_tmp;
            delete_index = frame_index;
            % num_match_most
            % start_frame
        end
    end
    assert(delete_index == start_frame);
    fprintf('start with pair %d\n', start_frame);
    mergedGraph = Graph{start_frame};
    graph_queue(delete_index) = [];

    % merge all
    for merge_index = 1:frames.length-2

        % find a new graph with most common points    
        num_pts_common_best = 0;
        for frame_index = 1:length(graph_queue)
            graph_tmp = graph_queue(frame_index);
            num_pts_common = obtain_common_pts(mergedGraph, Graph{graph_tmp});
            if num_pts_common > num_pts_common_best
                num_pts_common_best = num_pts_common;
                to_merge_frame = graph_tmp;
                delete_index = frame_index;               
            end
        end

        graph_queue(delete_index) = [];
        fprintf('merge with pair %d\n', to_merge_frame);
        mergedGraph = merge2graphs(mergedGraph, Graph{to_merge_frame});
        ...
        % bundle adjustment
        mergedGraph = bundleAdjustment(mergedGraph, adjust_type);
